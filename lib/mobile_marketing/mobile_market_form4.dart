...
class _mobile_Form4State extends State<mobile_Form4> {
                        ...
                        if(_formkey.currentState!.validate())
                        {
                          try{
                            ...
                            firestoreInstance.collection("Mobile Marketing Form4").doc(firebaseUser!.email).set(
                                {
                                  'Current_Work':CurrentWorkctlr.text,'Mobile_AdFit':MobileAdFitctlr.text,
  ...
  getdata()async{
    var firebaseUser =  FirebaseAuth.instance.currentUser;
    final data =firestoreInstance.collection("Mobile Marketing Form4").doc(firebaseUser!.email);
    final snapshot = await data.get();
    if(snapshot.exists){
      CurrentWorkctlr.text = snapshot['Current_Work'];
      MobileAdFitctlr.text= snapshot['Mobile_AdFit'];
